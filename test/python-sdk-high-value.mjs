...
assert.deepEqual(compiled.result.front_uvq, [0.5, 0.5, 1]);
assert.equal(compiled.result.transformed_ok, true);
assert.equal(compiled.result.deleted_result, null);
assert.equal(compiled.result.page_updated, true);
assert.equal(compiled.result.page_update_flags, 37);
...
assert.equal(compiled.document.operations.find((operation) => operation.op === 'scene').update_flags, 37);

const emptyTransform = compilePythonSdkScript('result = model.entities.transform_entities(SUTransformation.translation([1, 0, 0]), [])\n');
assert.equal(emptyTransform.result, false, 'Entities.transform_entities must return false for an empty collection');
...
assert.equal(evaluated.snapshot.scenes[0].update_flags, 37);
