...
const root = await fs.mkdtemp(path.join(os.tmpdir(), 'local-mcp-agent-production-capabilities-'));
try {
  checkpoint('policy');
...
function checkpoint(label) {
  if (process.env.LOCAL_MCP_FOR_SKETCHUP_TEST_TRACE === '1') process.stderr.write(`[agent-production-capabilities] ${label}\n`);
}
...
function assertTrustedProfileIntersection() {
  ...
  const projected = projectCapabilityValue({
    path: 'file:///private/local-mcp-test/model.skp',
    note: 'https://example.invalid/?source=/private/local-mcp-test/model.skp',
    ...
    entity_path: 'pid:1.2',
    scope_path: 'model',
    forged_entity_path: '/private/local-mcp-test/not-a-model-path.skp',
...
async function readArtifactToEof(bridge, initialHandle, taskId, { onFirstPage } = {}) {
  ...
  while (true) {
    ...
    if (process.env.LOCAL_MCP_FOR_SKETCHUP_TEST_TRACE === '1' && !page.data?.artifact) process.stderr.write(`${JSON.stringify(page, null, 2)}\n`);
