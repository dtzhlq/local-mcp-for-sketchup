...
const [evidence, schema, targetValidationSchema] = await Promise.all([
  readJson('docs/evidence/current-source-reviewed-plan-v3-live-evidence-2026-07-21.json'),
  readJson('schema/current-source-reviewed-plan-v3-live-evidence-v1.schema.json'),
...
const currentDriftBindings = [];
for (const binding of evidence.source_bindings) {
  assert.match(binding.sha256, /^sha256:[0-9a-f]{64}$/); assertions += 1;
  const currentHash = await sha256File(binding.path);
  if (currentHash !== binding.sha256.slice('sha256:'.length)) currentDriftBindings.push(binding.path);
}

assert.equal(evidence.source_binding_status, 'historical_exact_source_snapshot'); assertions += 1;
assert.ok(currentDriftBindings.includes('src/existing-model-edit-proposer.mjs')); assertions += 1;
assert.deepEqual(evidence.current_source_replacement, {
  target_selection_scope: 'docs/evidence/current-source-multi-model-target-quality-live-evidence-2026-07-22.json',
  reviewed_plan_scope: null
}); assertions += 1;
...
assert.equal(evidence.proposal.leaf_entities_materialized, false); assertions += 1;
assert.equal(evidence.proposal.selected_entity_path, evidence.reviewed_plan.selected_entity_path); assertions += 1;
assert.equal(evidence.reviewed_plan.plan_version, '2026-07-existing-model-edit-plan.3'); assertions += 1;
...
assert.equal(evidence.reviewed_plan.execution_target_validation_policy.mode, 'structural_groups'); assertions += 1;
assert.equal(evidence.reviewed_plan.execution_target_validation_policy.source, 'hash_bound_server_proposal'); assertions += 1;
assert.equal(evidence.reviewed_plan.execution_target_validation_policy.leaf_entities_materialized, false); assertions += 1;
...
assert.equal(evidence.acceptance.current_source_plan_v3_reviewed_preparation, false); assertions += 1;
...
assert.equal(evidence.acceptance.mock_six_phase_execution_contract, true); assertions += 1;
assert.equal(evidence.acceptance.live_post_approval_execution, false); assertions += 1;
...
const runRoot = 'output/live-validation/reviewed-plan-readonly/trimble-s6-bounded-reviewed-plan-v3-2026-07-21';
...
if (localCaptureArtifactsVerified === localArtifacts.length) {
  ...
  assert.equal(report.reviewed_plan.plan_version, evidence.reviewed_plan.plan_version); assertions += 1;
  assert.deepEqual(report.reviewed_plan.execution_target_validation_policy, evidence.reviewed_plan.execution_target_validation_policy); assertions += 1;
  ...
  assert.equal(proposalProjection.result.target_discovery.leaf_entities_materialized, false); assertions += 1;
  ...
  assert.deepEqual(
    reviewedProjection.result.approval_challenge.review_context.execution_target_validation,
    evidence.reviewed_plan.execution_target_validation_policy
  ); assertions += 1;
  assert.equal(reviewedProjection.result.approval_challenge.status, 'awaiting_trusted_user'); assertions += 1;
  assert.equal(plan.version, evidence.reviewed_plan.plan_version); assertions += 1;
  assert.equal(plan.plan_hash, evidence.reviewed_plan.plan_hash); assertions += 1;
  assert.equal(existingModelEditPlanHash(plan), plan.plan_hash); assertions += 1;
  ...
  assert.deepEqual(plan.target_validation, evidence.reviewed_plan.target_validation); assertions += 1;
  assert.deepEqual(existingModelEditExecutionTargetValidationPolicy(plan), evidence.reviewed_plan.execution_target_validation_policy); assertions += 1;
  const tamperedPlan = structuredClone(plan);
  tamperedPlan.target_validation.structural_group_limit -= 1;
  assert.notEqual(existingModelEditPlanHash(tamperedPlan), plan.plan_hash); assertions += 1;
...
const negativeCases = [
  mutate(evidence, (value) => { value.source_binding_status = 'current_exact_source_snapshot'; }),
  mutate(evidence, (value) => { value.current_source_replacement.reviewed_plan_scope = 'docs/evidence/not-a-current-proof.json'; }),
  ...
  mutate(evidence, (value) => { value.reviewed_plan.plan_version = '2026-07-existing-model-edit-plan.2'; }),
  mutate(evidence, (value) => { value.reviewed_plan.target_validation.exact_targets_verified = false; }),
  mutate(evidence, (value) => { value.reviewed_plan.execution_target_validation_policy.mode = 'full_recursive'; }),
  mutate(evidence, (value) => { value.reviewed_plan.execution_target_validation_policy.leaf_entities_materialized = true; }),
  ...
  mutate(evidence, (value) => { value.acceptance.live_post_approval_execution = true; }),
  mutate(evidence, (value) => { value.acceptance.release_acceptance = true; })
];
for (const invalid of negativeCases) {
  assert.equal(validate(invalid), false, 'unsafe or overclaimed plan v3 evidence must fail schema validation'); assertions += 1;
}

process.stdout.write(`${JSON.stringify({
  ok: true,
  evidence_scope: evidence.evidence_scope,
  plan_version: evidence.reviewed_plan.plan_version,
  ...
  execution_policy_disclosed: true,
  source_binding_status: evidence.source_binding_status,
  current_drift_bindings: currentDriftBindings,
  current_reviewed_plan_replacement: false,
  live_post_approval_execution: false,
