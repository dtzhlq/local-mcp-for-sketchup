...
plugin_dir = File.join(repo_root, 'sketchup_plugin', 'alma_sketchup_mcp')
...
tests = 0
boolean_expected = AlmaSketchupMCP::RuntimeSourceManifest::BOOLEAN_OPERATIONS_SHA256
boolean_actual = Digest::SHA256.file(File.join(plugin_dir, 'boolean_operations.rb')).hexdigest
model_revision_expected = AlmaSketchupMCP::RuntimeSourceManifest::MODEL_REVISION_SHA256
...
Dir.mktmpdir('alma-runtime-source-attestation-') do |root|
  source = File.join(root, 'boolean_operations.rb')
  FileUtils.cp(File.join(plugin_dir, 'boolean_operations.rb'), source)
  yielded_path = nil
  loaded = AlmaSketchupMCP::RuntimeSourceAttestation.attest_boolean_operations!(root) do |path|
    yielded_path = path
    true
  end
  ...
  tests += 1; check!(AlmaSketchupMCP::RuntimeSourceAttestation.loaded_boolean_operations_sha256 == boolean_expected, 'loaded hash must remain available after the load boundary')

  tests += 1
  expect_load_error('an already-loaded require no-op must fail closed') do
    AlmaSketchupMCP::RuntimeSourceAttestation.attest_boolean_operations!(root) { false }
  end

  File.open(source, 'ab') { |file| file.write("# tampered before load\n") }
  tests += 1
  expect_load_error('pre-load source drift must fail closed') do
    AlmaSketchupMCP::RuntimeSourceAttestation.attest_boolean_operations!(root) { nil }
  end

  FileUtils.cp(File.join(plugin_dir, 'boolean_operations.rb'), source)
  tests += 1
  expect_load_error('source changes across the require boundary must fail closed') do
    AlmaSketchupMCP::RuntimeSourceAttestation.attest_boolean_operations!(root) do |path|
...
Dir.mktmpdir('alma-model-revision-source-attestation-') do |root|
  source = File.join(root, 'model_revision.rb')
  FileUtils.cp(File.join(plugin_dir, 'model_revision.rb'), source)
  yielded_path = nil
  loaded = AlmaSketchupMCP::RuntimeSourceAttestation.attest_model_revision!(root) do |path|
    yielded_path = path
    true
  end
  ...
  tests += 1; check!(AlmaSketchupMCP::RuntimeSourceAttestation.loaded_model_revision_sha256 == model_revision_expected, 'loaded Model Revision hash must remain available after the load boundary')

  tests += 1
  expect_load_error('an already-loaded Model Revision require no-op must fail closed') do
    AlmaSketchupMCP::RuntimeSourceAttestation.attest_model_revision!(root) { false }
  end

  File.open(source, 'ab') { |file| file.write("# tampered before load\n") }
  tests += 1
  expect_load_error('Model Revision pre-load source drift must fail closed') do
    AlmaSketchupMCP::RuntimeSourceAttestation.attest_model_revision!(root) { nil }
  end

  FileUtils.cp(File.join(plugin_dir, 'model_revision.rb'), source)
  tests += 1
  expect_load_error('Model Revision source changes across the require boundary must fail closed') do
    AlmaSketchupMCP::RuntimeSourceAttestation.attest_model_revision!(root) do |path|
      File.open(path, 'ab') { |file| file.write("# changed during load\n") }
      true
    end
  end
