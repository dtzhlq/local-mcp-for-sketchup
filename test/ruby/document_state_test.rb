...
require_relative '../../sketchup_plugin/alma_sketchup_mcp/document_state'

def AlmaSketchupMCP.camera_snapshot(camera)
...
first_state = AlmaSketchupMCP.document_state(first)
...
AlmaSketchupMCP.persist_document_state(first)

second_state = AlmaSketchupMCP.document_state(second)
second_state['scenes'] << { 'name' => 'Second Scene' }
second_state['style_state'] = { 'name' => 'Second Style' }
AlmaSketchupMCP.persist_document_state(second)

assert_equal(['First Scene'], AlmaSketchupMCP.document_state(first)['scenes'].map { |scene| scene['name'] }, 'first document scenes must remain isolated')
assert_equal(['Second Scene'], AlmaSketchupMCP.document_state(second)['scenes'].map { |scene| scene['name'] }, 'second document scenes must remain isolated')
assert_equal(nil, AlmaSketchupMCP.document_state(first)['style_state'], 'second document style must not leak into first')
...
reopened_state = AlmaSketchupMCP.document_state(reopened_first)
...
page_truth_scenes = AlmaSketchupMCP.snapshot_scenes(page_truth_model)
...
AlmaSketchupMCP.record_activated_model(second)
assert_equal(first, AlmaSketchupMCP.queue_active_model, 'focused Sketchup.active_model must replace a stale observer hint')
Sketchup.active_model = second
assert_equal(second, AlmaSketchupMCP.queue_active_model, 'queue routing must follow a live MDI focus switch in either direction')
AlmaSketchupMCP.instance_variable_set(:@queue_request_model, first)
assert_equal(first, AlmaSketchupMCP.queue_active_model, 'a queue request must keep one latched model for guard and operation consistency')
AlmaSketchupMCP.instance_variable_set(:@queue_request_model, nil)
AlmaSketchupMCP.forget_document_model(second)
Sketchup.active_model = first
assert_equal(first, AlmaSketchupMCP.queue_active_model, 'closed observed model must fall back to Sketchup.active_model')

corrupt = FakeModel.new(guid: 'corrupt-guid')
corrupt.set_attribute('AlmaSketchupMCP', 'document_state_v1', '{not-json')
corrupt_state = AlmaSketchupMCP.document_state(corrupt)
...
observer = AlmaSketchupMCP.install_document_activation_observer
assert_truthy(observer.is_a?(AlmaSketchupMCP::DocumentActivationObserver), 'document activation observer must install')
