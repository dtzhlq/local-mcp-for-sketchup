...
const assertions = [
  ...
  check('transform return contract', result.compiled?.result?.transformed_ok === true, result.compiled?.result?.transformed_ok),
  check('erase return contract', result.compiled?.result?.deleted_result === null, result.compiled?.result?.deleted_result),
  check('page update return contract', result.compiled?.result?.page_updated === true, result.compiled?.result?.page_updated),
  check('page named update flags', result.snapshot?.scenes?.[0]?.update_flags === 37, result.snapshot?.scenes?.[0]?.update_flags),
  check('affine front UVQ result', JSON.stringify(result.compiled?.result?.front_uvq) === JSON.stringify([0.5, 0.5, 1]), result.compiled?.result?.front_uvq),
