...
- R11/R11.2 已把 bird-eye land-cover、source-image BoundaryGraph v1、HighContrastEdge v1、真实 OpenCV Edge v1、INSID3 optional compare contract、R11-aware AutoGroundPlan、aggressive completion、四联/边线/消融 QA artifact 和 synthetic bird-eye/boundary fixture 加入 `npm run test:image-structured`；当前没有样本被标成 `photo_grade_readiness=candidate`，阻塞项继续通过 readiness/correction targets 暴露。
...
- 下一步：先把 OpenCV 边线从“候选线段”推进到更稳定的语义边界分类和 view/ground-plane estimate，再扩展更多鸟瞰/轴测鸟瞰图类型样本；重点降低右侧/底部边界、道路边线和铺装/绿化分界在遮挡情况下的 width/snap residual，而不是再靠 layout prior 补洞。同时把 `building-real-photo-smoke` 的 placeholder 输入替换成真实 top/oblique 建筑照片，或继续优化现有 building-group/second-sample 的 oblique facade height cues。在 ScaleAnchorGraph、GroundPlan、line/grid residual、TopViewOverlay、oblique/facade 和 promotion gate 同时成立前，cars、trees、roof vents、facade openings 等 dense helper classes 不能晋级为看似完整的真实几何。
