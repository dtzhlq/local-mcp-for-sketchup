...
7. R7 输入基线已推进到 review-gated massing + mock/live queue QA：`test/建筑群/` 的三张 GPT Image 合成航拍图已接入 `building_group` profile，`image-structured:build-building-group` 会生成 `examples/building-group/observations.json`、review overlays、`part-graph.massing.json` 和 `output.massing.json`。尺度来自停车位、车道和人行横道等 known-element anchors，约 `130m x 98m`，但所有 shape proposals 仍保持 review-gated。`image-structured:qa-building-group` 已补 mock layout QA 和 Reference Visual QA 报告；`image-structured:qa-building-group:queue` 已补 live queue QA 并保存建筑群 `.skp`。
...
1. 继续 R7 建筑群照片建模：基于已生成的 `building_group` massing PartGraph 和 mock/live queue layout/reference QA，补更细的屋顶线/立面/开窗门洞 correction targets。
