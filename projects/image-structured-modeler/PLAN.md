...
1. 进入图片结构化基础能力加固：R11.2 已把当前 building-group 的地面分割入口从屋顶/parking prior 直接推平面，改为先跑 local classical CV land-cover，再用 source-image detector、HighContrastEdge v1 和真实 OpenCV Edge v1 生成可追踪的 observed/completed/extrapolated edges、rejected strong-edge alternatives 和 inferred road corridors；INSID3 仍只是 optional backend contract，默认 skipped，不进入测试重依赖，也不应被描述成已实际使用。下一步不是继续堆 prior，而是把 OpenCV/HighContrast 边线做成稳定的语义边界：区分建筑外轮廓、屋面内部缝、道路边线、铺装/绿化分界和 site perimeter，并补 view/ground-plane estimate，让遮挡猜测建立在可校准的 observed anchors 上。之后再扩展更多鸟瞰图/轴测鸟瞰样本；真实照片 smoke 仍需要替换为真实 top/oblique 建筑照片资产或补更强 oblique facade/height cues，目标是在不绕过 scale、GroundPlan、line/grid residual、TopViewOverlay、oblique/facade 和 promotion gate 的前提下尝试产出第一个 `photo_grade_readiness=candidate`。
