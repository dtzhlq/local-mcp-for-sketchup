...
7. R7.0 输入基线已完成：`test/建筑群/` 的三张 GPT Image 合成航拍图已接入 `building_group` profile，`image-structured:analyze-building-group` 会生成 `examples/building-group/observations.json` 和 review overlays，并把 top/oblique 视角、site/building/road/parking/tank evidence 和 orientation review gate 纳入 `test:image-structured`。
...
1. 继续 R7 建筑群照片建模：基于已生成的 `building_group` EvidenceGraph，把建筑体块、立面平面、屋顶线、开窗/门洞、尺度锚点和遮挡/缺视角问题写成 review-gated massing PartGraph。
