...
>
> 2026-06-25 主线补充：official API parity R1 已把本地 JSON DSL 推到 90 个 operation / 56 个 component scope，新增 `geometry_input`、`curve`、`arc_curve`、`image_reference`、`face_uv`；MCP/CLI/bridge/mock/queue 新增 active model inspection（`inspect_model` / `list_entities` / `get_model_info`）、selection（`get_selection` / `set_selection`）、file lifecycle（`open_model` / `import_model` / `export_model` / `save_model_version`）、受控 `evaluate_py` 兼容层和 `build_report` artifact 包。`evaluate_py` 只执行 JSON DSL、restricted Expert Mode 或 gated Ruby expert debug，不声明完整官方 Python SDK；live queue 已完成 R1 capability、build、inspection、selection、save、evaluate_py、build_report、export/import 和 `open_model` smoke。
>
> 2026-07-02 主线补充：仓库内部 restricted facade expression R3（不是 SketchUp 官方 API 版本）已把本地 JSON DSL 推到 91 个 operation / 56 个 component scope，新增 runtime `selection` operation，并把 restricted Python SDK facade 补到 `Face.followme`、`Face.mesh`、`PolygonMesh`、`Entities.add_faces_from_mesh/fill_from_mesh`、`Face.position_material`、runtime Selection 和 Page transition/layer/object visibility/rendering/shadow 字段。`examples/python-sdk-official-api-expression-r3-fixture.py` 已通过 mock `evaluate_py input_format=python_sdk`：12 operations / 5 groups / 16 faces / warnings 0，snapshot 回传 `followme_realized`、`face_uvs.mapping`、`selection` 和 scene advanced fields。live queue `npm run qa:official-api-r3:queue` 已在 SketchUp 2026 / plugin `queue-plugin-0.1.0-official-api-expression-r3` 上通过：12 operations / 5 groups / 10 faces / 29 edges / 25 vertices / selection 1 / scene 1 / warnings 0，并保存 `output/python-sdk-official-api-expression-r3.skp`。
>
> 2026-07-02 主线补充（历史节点）：自然迭代体验第一刀新增 `iterate_model`。该工具不重构 runtime，而是在现有 active inspection / selection / `evaluate_py` / snapshot diff / QA / save_model_version / capture_view 之上加一层可审计编排：记录 before/after snapshot、change summary、snapshot diff、QA、manifest 和 versioned model artifact。当日 MCP 工具数为 28；随后 `ModificationIntent v1` 等入口把当前 `tools/list` 推进到 32。mock/MCP 回归覆盖 active session 增量 patch 与 artifact 落盘。
>
...
>
> 2026-06-25 表达力 P0.1 Face/Loop/Edge 第一刀：restricted Python SDK facade 现在能从 `GeometryInput.add_face/add_edge` 返回 Face/Edge facade，并通过 `face.edges`、`face.loops`、`face.outer_loop` 暴露 Edge/Loop facade；覆盖 `face.vertices/edges/loops/outer_loop/normal/plane/area`、`face.reverse()`、正反材质、Face metadata、`loop.is_outer()/to_indices()`、`edge.start/end/vertices/length/to_indices()`。当时 `face.pushpull(...)` 只写入 metadata-only intent；2026-07-02 的 Official API coverage R2 已把默认 facade pushpull 推进为真实几何效果，并保留显式 `metadata_only=True` 的 review-only 路径。
>
> 2026-06-25 表达力 P0.2 Component facade 第一刀：restricted Python SDK facade 新增 `ComponentDefinition` / `ComponentInstance` 对象包装现有 component DSL。当前支持 definition 收集 nested `GeometryInput` / `Group`、`definition.create_instance()`、直接 `ComponentInstance(...)`、definition object 引用、instance origin 单位换算和 transform 转译；这是组件复用表达力增强，不是完整 SketchUp 组件编辑/definition browser API。
>
> 2026-06-25 表达力 P0.3 View facade 第一刀：restricted Python SDK facade 新增 `Camera` / `Scene` / `Style` / `ShadowInfo` / `RenderingOptions` 对象包装现有 view/appearance DSL。当前支持 camera/scene inches->mm、`camera.look_at()`、style color 字段、shadow 和 rendering options 转译；这是视图与表现层表达力增强，不是完整 SketchUp Style/Rendering API。
>
> 2026-06-25 表达力 P0.4 Appearance facade 第一刀：restricted Python SDK facade 新增 `Layer` / `Texture` / `Image` / `ImageRep` 对象包装现有 tag/material/image asset DSL。当前支持 `Layer` -> `tag`、`assign_layer` -> `assign_tag`、`Texture` -> `Material.texture`、`Image` / `ImageRep` -> `image_reference`；这是外观资源表达力增强，不是完整贴图编辑、嵌入图像或 UV mapping API。
>
> 2026-06-25 表达力 P0.5 Curve facade 第一刀：restricted Python SDK facade 新增 `Curve` / `ArcCurve` 对象包装现有 `curve` / `arc_curve` DSL。当前支持对象构造、`model.add_curve/add_arc_curve`、source units 换算和 zero-face edge geometry snapshot；这是曲线表达力增强，不是完整 SketchUp Curve/ArcCurve 编辑 API。
>
> 2026-06-26 表达力 P1 helper function 第一刀：restricted Python SDK facade 现在支持受限 `def` / `return`、局部作用域、位置参数、关键字参数和默认参数，可把重复建模片段封装为 Python 风格 helper，再转译为既有安全 JSON DSL。该能力不执行 Python bytecode，仍阻断 import、print/runtime/file/network 访问、decorator、nested closure、varargs/kwargs、异常处理、动态反射和直接 SketchUp API 调用；新增 `examples/python-sdk-helper-functions-fixture.py` 覆盖 helper 生成三块 GeometryInput 面板并返回 Face facade。
>
> 2026-07-02 表达力 P2 structured Python expression 第一刀：restricted Python SDK facade 新增 list/dict comprehension、tuple/list destructuring、dict `keys/values/items/get/update`、`list/tuple/dict/enumerate/zip/sorted/sum/reversed/all/any` 受限 builtins、dict/string/list iterable、负索引和简单 slice。该能力让批量面板、材质表、规格表驱动建模更接近官方 Python SDK 的脚本表达方式，但仍只解释 Python AST 并转译为安全 JSON DSL；新增 `examples/python-sdk-comprehension-fixture.py` 覆盖 dict comprehension 生成材质表、list comprehension 生成面板和 `enumerate/zip/sum/sorted/list(dict.keys())` 的建模数据流。
>
> 2026-07-02 Official SketchUp API coverage R2：新增 `docs/sketchup-ruby-api-coverage-matrix.md` 和 `examples/python-sdk-official-api-coverage-fixture.py`。restricted Python SDK facade 现在支持 `model.entities/materials/layers/definitions/pages/selection/active_view`、`Entities.add_face/add_edges/add_line/add_curve/add_arc/add_circle/add_group/add_instance/add_3d_text`、object `set_attribute/layer=/material=/visible=/transform_by/delete`、`RenderingOptions` / `ShadowInfo` key-value access 和 `keys/each_key/each_pair`。`Face.pushpull(...)` 默认从 metadata-only 推进为真实几何效果：mock snapshot 反映 bbox/faces/edges/vertices，queue runtime 调 SketchUp `Face#pushpull`；显式 `metadata_only=True` 仍保留 review-only intent。live queue `output/python-sdk-official-api-coverage-queue.json` 已复验为 18 operations / 2 groups / 1 instance / 18 faces / 36 edges / 24 vertices / warnings 0。R3 已继续补上 followme、mesh/fill、position_material、Selection runtime 和 Page 高级字段第一刀。
...
因此当前发布状态仍为：**技术预览可演示，正式发布暂停**。主线受控 feature operations、CAD boolean/manifold 与 queue active model 防护已补齐；Image Structured Modeler 已补齐 observation/model-plan/review evidence graph、`review.semantic_fusion` 跨图语义融合层、交互式 corrections workbench，以及 compact remote 的真实 feature mapping 第一刀。R5 已让救护车、Switch 与 Fuji 进入 `ProductProfile -> PartGraph -> DSL -> Layout QA + Reference Visual QA` 样本报告，并通过 mock + live queue 批量 gate；R6 已把 ambulance no-seed 图像证据转成 review-gated 参数提案，把已接受提案转成 correction patch，提供 proposal review HTML 来导出 accepted proposal JSON，并新增 proposal-applied mock/queue 复验链来证明部分接受的 no-seed skeleton 仍会被 QA 留在 review-gated 状态。Physical consistency QA 已把 ambulance、Switch、Fuji 的支撑/贴合/接地关系加入 PartGraph gate，并通过漂移负例验证 `update_part_graph` correction target。Reference Visual QA 现在会显式检查 orientation/chirality 和 accepted feature presence/count，图像 observation 也会记录左右/前后 mirror-risk hints，避免把视觉识别里的左右判断静默传成模型布局。R7 已把 GPT Image 生成的建筑群三视角输入纳入 `building_group` observation/evidence/overlay 回归，并基于停车位、车道和人行横道等已知要素估算出约 `130m x 98m` 的厂区尺度，生成 review-gated massing PartGraph、DSL、mock/live queue layout QA 和 Reference Visual QA，并保存 `output/image-structured-building-group-massing.skp`；R7 final 接受全部 5 个当前 roofline/facade/opening detail proposal target，生成 `part-graph.r7-final.json` / `output.r7-final.json`，通过 mock/live queue layout、feature Reference Visual QA 与 physical consistency，并保存 `output/image-structured-building-group-r7-final.skp`。2026-06-01 的新增切片把 image-space VisualRelationGraph 写入 ObservationSet / EvidenceGraph，Switch 的人工视觉检查变成 `visual-relations.fixture.json` + `image-structured:visual-relation-switch`，并将建筑群 relation evidence 回灌到 massing PartGraph 的 `relationships` 与可检查的 spatial `physical_relations`；四栋仓库、停车线/车道和树列现在作为 review-gated candidate parts 进入 proposal queue。accepted fixture 现在 promotion 全部 8 个候选为真实 `parts[]`，其中蓝屋顶和仓库屋顶使用 `pixel_color_segmentation`，停车线/车道使用 `pixel_line_segmentation`，树列使用 `pixel_vegetation_segmentation`；candidate fixture 检查 12 条视觉关系和 9 个 footprint 的模型投影 center/extent，当前 22-part PartGraph / 20-group DSL 已通过 mock/live queue layout、reference、visual relation/footprint、physical QA。这仍不是照片级或测绘级重建。
...
| 文件 | 职责 |
|---|---|
| `src/mcp-server.mjs` | stdio MCP server，暴露 32 个工具 |
...
| `src/appearance-operations.mjs` | mock runtime image_reference 与 face_uv 元数据操作 |
| `src/primitive-operations.mjs` | mock runtime mesh、geometry_input、curve、arc_curve、prism、cylinder 和 profile plane/prism vertices helper |
| `src/model-inspection.mjs` | mock/bridge 共用 active model inspection、entity list、selection 和 version path helper |
| `src/modification-intent.mjs` | `plan_modification_intent` 意图层：selection/geometry/target evidence → ModificationIntent → 可选 JSON DSL patch 和确认门控 |
| `src/iteration.mjs` | `iterate_model` 自然迭代薄层：支持 code 或安全 ModificationIntent，输出 before/after snapshot、change summary、snapshot diff、QA、manifest、版本保存和可选 queue capture |
...
| `src/architecture-operations.mjs` | mock runtime 建筑 helper：level、slab、wall/path/roof/site、stairs/railing |
...
| `docs/modeling-expression-status.md` | 主线建模表达能力现状、restricted Python SDK facade P0-P2 和官方 Connector 差距 |
| `docs/sketchup-ruby-api-coverage-matrix.md` | Official SketchUp API coverage R2/R3 覆盖矩阵：Ruby API 高频类、facade 映射和 blocked/out-of-scope 边界 |
...
| `test/python-sdk-compiler.mjs` | Python SDK facade 编译、`evaluate_py python_sdk` mock build 和安全拒绝测试 |
...
| **done** | R7.0 建筑群输入基线 | `test/建筑群` 三张 GPT Image 合成图已接入 `building_group` profile，生成 `examples/building-group/observations.json` 与 review overlays，并纳入 `test:image-structured` |
| **done** | R7 known-element scale + massing PartGraph | 停车位、车道和人行横道尺度锚点已写入 `scale_calibration`，并生成 review-gated `part-graph.massing.json` / `output.massing.json` |
| **done** | R7 建筑群 mock QA | 新增 `examples/model-qa/building-group-massing.json` 与 `examples/reference-visual-qa/building-group-massing.json`，`image-structured:qa-building-group` 输出 layout/reference 报告并通过 |
| **done** | R7 建筑群 live queue QA | `image-structured:qa-building-group:queue` 已通过 queue layout/reference QA，并保存 `output/image-structured-building-group-massing.skp` |
| **done** | R7 建筑细节 proposal 第一刀 | `part-graph.massing.json` 已新增 roofline/facade/opening `feature_intents` proposals；`image-structured:proposal-review-chain-building-group` / `:queue` 已完成 accepted subset -> patch -> mock/live queue QA |
| **done** | R7 建筑群 final detail gate | `image-structured:proposal-review-chain-building-group-all` / `:queue` 已完成 accepted-all detail proposals -> patch -> `part-graph.r7-final.json` -> `output.r7-final.json` -> layout + feature Reference Visual QA + physical consistency，并保存 `output/image-structured-building-group-r7-final.skp` |
| **done** | VisualRelationGraph 第一刀 | ObservationSet 已输出 image-space relation candidates；Switch/building-group 视觉关系 fixture + `mirror_x` 负例已进入 `test:image-structured`，建筑群 massing PartGraph 已接收 relation evidence，输出 review-gated candidate part proposals，并已完成四栋仓库、停车线/车道和树列 pixel-grounded candidate promotion mock/live queue QA |
| **done** | R9.5 PhotoGradeReadiness | 新增六 gate readiness report，覆盖 scale、subdivision、line/grid、top-view overlay、oblique/facade 和 promotion；building-group 与 second sample 为 `technical_baseline`，real-photo smoke scaffold 为 `review_required` |
...
- **2026-07-03**：自然迭代体验 R2 新增 `ModificationIntent v1`：`plan_modification_intent` 会把 target resolution、current selection 和 `analyze_selection_geometry` 几何事实转成可审计 edit intent，记录 evidence refs、confidence、`requires_confirmation`、`safe_to_execute`、limitations 和可选 JSON DSL patch。`iterate_model` 现在可直接消费 `intent` / `intent_file`；只有 `ok=true`、`safe_to_execute=true`、`requires_confirmation=false` 才执行，否则写 preview/blocked manifest 并停止。MCP tools/list 当前为 32 个工具。live queue smoke 已通过：Face+Edge selection 只生成 preview/blocked intent；top-level group 安全 `set_attribute` intent 可执行并保存 versioned `.skp` artifact。
- **2026-07-03**：RhinoMCP-inspired `ParametricRecipe` / `FeatureMappingGraph` 第一刀完成：新增 `schema/parametric-recipe.schema.json`、`src/product-modeling/parametric-recipe.mjs`、Switch thumbstick recipe fixture 和 expected `FeatureMappingPlan` artifact。当前只做静态 schema/compile/report 与 `first_output_then_fanout` fail-closed gate，不接开放脚本执行，也不声明完整 Grasshopper 替代；后续 TODO 是在 QA 证据成熟后编译到 reviewed PartGraph / safe JSON DSL。
- **2026-07-03**：主线 release gate 复核补齐路径可移植性修复：PartGraph/photoreal DSL artifacts 内保留 repo-relative asset path，queue runtime 发送前通过 `materializeDslAssetPaths` 物化为绝对路径，解决 worktree checkout 路径漂移导致的 `npm test` / `npm run test:image-structured` 失败。release checklist 同步到 91 ops / 56 component scope，并纳入 `qa:official-api-r3:queue`。
- **2026-07-02**：自然迭代体验第一刀完成并通过 mock 回归：新增 `src/iteration.mjs`、Bridge/CLI/MCP/HTTP `iterate_model` 入口和 workflow `iterator`。`iterate_model` 会 inspect active session、可选 `set_selection`、通过现有 `evaluate_py` 执行增量 patch，再写 before/after snapshot、change summary、snapshot diff、可选 QA、manifest、versioned model artifact 和可选 queue capture。已通过 JS syntax、`node test/mcp-server.mjs`、`node test/mock-validation.mjs`、CLI mock smoke、`npm test`、`npm run plugin:check` 和 `git diff --check`；MCP tools/list 现在包含 `iterate_model`，工具数为 28。
- **2026-07-02**：Official SketchUp API expression R3 第一刀完成 mock + live queue 验证：新增 `examples/python-sdk-official-api-expression-r3-fixture.py` 和 `scripts/validate-official-api-expression-r3.mjs`，`src/python-sdk-compiler.mjs` 支持 `PolygonMesh`、`Face.followme`、`Face.mesh`、`Entities.add_faces_from_mesh/fill_from_mesh`、`Face.position_material/clear_texture_position/texture_positioned`、runtime `selection` op 和 Page transition/layer/object visibility/rendering/shadow 字段；`src/capabilities.mjs` manifest 推进到 `2026-07-official-api-expression-r3` / capability `0.1.0-capabilities.10`，当前 91 ops / 56 component scope。已通过 JS/Ruby syntax、registry 生成、`node test/python-sdk-compiler.mjs`、`npm test`、`npm run plugin:check`、`git diff --check`、`npm run qa:official-api-r3:mock` 和 `npm run qa:official-api-r3:queue`。R3 mock snapshot 为 5 groups / 16 faces / warnings 0；R3 queue snapshot 为 5 groups / 10 faces / 29 edges / 25 vertices / selection 1 / scene 1 / warnings 0，queue JSON 为 `output/python-sdk-official-api-expression-r3-queue.json`，真实 `.skp` 为 `output/python-sdk-official-api-expression-r3.skp`（143060 bytes）。queue 复验证明 `followme` 形成 38.1mm 高 bbox、`position_material` mapping 写入 `geometry_input` metadata、runtime selection snapshot 回传 1 个对象、scene transition/layer/object visibility/rendering/shadow 字段回传。
- **2026-07-02**：Official SketchUp API coverage R2 第一刀完成 mock 验证：新增 `docs/sketchup-ruby-api-coverage-matrix.md` 和 `examples/python-sdk-official-api-coverage-fixture.py`，`src/python-sdk-compiler.mjs` 支持 `model.entities/materials/layers/definitions/pages/selection/active_view`、`Entities.add_face/add_edges/add_line/add_curve/add_arc/add_circle/add_group/add_instance/add_3d_text`、object `set_attribute/layer=/material=/visible=/transform_by/delete`、`RenderingOptions` / `ShadowInfo` key-value access 和 `keys/each_key/each_pair`；`src/primitive-operations.mjs` 与 Ruby `primitive_operations.rb` 将 facade `Face.pushpull` 默认推进为真实几何效果，显式 `metadata_only=True` 保留 review-only intent。已通过 `node --check src/python-sdk-compiler.mjs`、`node --check src/primitive-operations.mjs`、`ruby -c sketchup_plugin/alma_sketchup_mcp/primitive_operations.rb`、`node test/python-sdk-compiler.mjs` 和 mock `evaluate_py`；R2 mock snapshot 为 2 groups / 1 instance / 18 faces / warnings 0，artifact 为 `output/python-sdk-official-api-coverage-mock.json`。
- **2026-07-02**：表达力 P2 structured Python expression 第一刀完成并通过 mock + live queue 验证：新增 `examples/python-sdk-comprehension-fixture.py`，`src/python-sdk-compiler.mjs` 支持 list/dict comprehension、tuple/list destructuring、dict `keys/values/items/get/update`、`list/tuple/dict/enumerate/zip/sorted/sum/reversed/all/any`、dict/string/list iterable、负索引和简单 slice；`test/python-sdk-compiler.mjs` 新增 P2 断言。已通过 `node test/python-sdk-compiler.mjs`、CLI `compile_python_sdk`、mock `evaluate_py`、`git diff --check`、`npm test`、`npm run plugin:check`、live queue `get_capabilities` compatibility `ok` 和 queue `evaluate_py input_format=python_sdk`。P2 queue snapshot 为 3 groups / 3 faces / 12 edges / warnings 0，artifact 为 `output/python-sdk-comprehension-queue.json`。
- **2026-07-02**：新增 `docs/modeling-expression-status.md`，按官方 Connector v1 当前公开口径整理主线建模表达能力现状和差距：官方仍是 `get_docs` / `evaluate_py` / `save_model` 的云端 `.skp` 生成闭环；本地当前是 91-op 安全 JSON DSL + restricted Python SDK facade P0-P2 + mock/queue runtime + active inspection/file lifecycle/report；差距集中在云端认证/session/download URL、完整 Python runtime、完整 SketchUp API、通用 CAD kernel 和图像到结构化模型上游集成。
- **2026-06-25**：表达力 P0/P0.1/P0.2/P0.3/P0.4/P0.5 第一刀完成：新增 `src/python-sdk-compiler.mjs`、`examples/python-sdk-facade-fixture.py`、`examples/python-sdk-face-edge-loop-fixture.py`、`examples/python-sdk-component-facade-fixture.py`、`examples/python-sdk-view-facade-fixture.py`、`examples/python-sdk-appearance-facade-fixture.py`、`examples/python-sdk-curve-facade-fixture.py` 和 `test/python-sdk-compiler.mjs`，并在 bridge/CLI/MCP 接入 `compile_python_sdk` 与 `evaluate_py input_format=python_sdk`。当前 facade 只解析 Python AST，不执行 Python；可把 `model`、`Material`、`SUColor`、`SUPoint3D`、`LoopInput`、`GeometryInput`、`Face` / `Loop` / `Edge` 返回对象、`Curve` / `ArcCurve` 第一刀、`ComponentDefinition` / `ComponentInstance` 第一刀、`Camera` / `Scene` / `Style` / `ShadowInfo` / `RenderingOptions` 第一刀、`Layer` / `Texture` / `Image` / `ImageRep` 第一刀、`SUTransformation` 等官方风格调用转为 JSON DSL，并支持 source units `inches` 自动换算到 DSL `mm`。Face/Loop/Edge fixture 覆盖 face/edge/loop 查询、`face.reverse()`、正反材质、metadata 和 metadata-only `face.pushpull` intent，mock snapshot 返回 1 group / 1 face / warnings 0、`geometry_input.edge_count=8`、`loop_count=2`；Component fixture 覆盖 nested `GeometryInput` definition、两个 instances、origin 换算和 transform；View fixture 覆盖 camera/scene/style/shadow/rendering options；Appearance fixture 覆盖 layer/tag、texture material、image references 和 tagged group；Curve fixture 覆盖 curve/arc_curve 对象；mock snapshot warnings 0；`import`、`print` 和任意 Python runtime 仍 fail-closed / blocked。
- **2026-06-25**：主线 official API parity R1 完成并完成 live queue 复验：manifest 推进到 `2026-06-official-api-parity-r1` / capability `0.1.0-capabilities.9` / plugin `queue-plugin-0.1.0-official-api-parity-r1`，operation 数推进到 `90`、component scope 推进到 `56`。新增 `geometry_input`、`curve`、`arc_curve`、`image_reference`、`face_uv`，并在 MCP/CLI/bridge/mock/queue 层新增 active model inspection、selection、file lifecycle、受控 `evaluate_py` 和 `build_report` artifact 包。已通过 `npm test`、`npm run registry:check`、`npm run plugin:check`、Ruby syntax、JS syntax、`git diff --check`、`npm run plugin:install`、live `get_capabilities` compatibility `ok`、新增 5 op queue build、`inspect_model` / `list_entities` / `get_model_info`、`set_selection` / `get_selection`、`save_model_version`、queue `evaluate_py`、queue `build_report`、`export_model`、`.skp` `import_model` 和 `open_model` smoke；`.skp` 导入采用 `definitions.load + explode` 并在导入后无可见模型变化时 fail-closed，`open_model` 在 active model 未同步切到目标 path 时 fail-closed。
- **2026-06-09**：主线 Building Geometry R3 quality slice 第一刀完成：manifest 推进到 `2026-06-building-geometry-r3-quality` / capability `0.1.0-capabilities.8`，operation 数保持 `85`、component scope 保持 `53`。`curtain_wall` 从最小墙带升级为 module grid，支持 `row_count`、`panel_thickness`、vertical mullions、horizontal rails 和 panel cells；mock/Ruby queue contract 均标记 supported。
- **2026-06-04**：主线 Building Geometry R2 aggressive slice 完成 mock 大推进和 live queue 抽样：manifest 推进到 `2026-06-building-geometry-r2-aggressive` / capability `0.1.0-capabilities.7` / plugin `queue-plugin-0.1.0-building-geometry-r2.1`。`footprint_slab` 支持 holes，非轴向 `wall` 和 `wall_path` 支持 segment-local openings，并新增 `curved_wall`、`roof_footprint`、`hip_roof`、`parapet_path`、`curtain_wall`、`column_grid`、`path_surface`、`terrain_mesh`、`parking_stall_array`。新增 `examples/building-geometry-r2-aggressive.json`，mock snapshot 为 12 groups / 408 faces / 779 edges / 446 vertices / warnings 0；queue snapshot 为 12 groups / 411 faces / 772 edges / 422 vertices / warnings 0，并保存 `output/building-geometry-r2-aggressive.skp`（190,893 bytes）。已通过 `npm test`、`npm run qa:mock`、`npm run plugin:check`、`npm run plugin:install`、`git diff --check`、aggressive mock build、live queue `get_capabilities`、aggressive queue build 和 `.skp` save；queue 侧对部分场地/幕墙 helper 仍是 partial/minimal 实现，不声明所有拓扑与 mock 完全同构。
- **2026-06-02**：R9.5 PhotoGradeReadiness 完成：新增 `projects/image-structured-modeler/scripts/lib/photo-grade-readiness.mjs`、`validate-photo-grade-readiness.mjs` 和 `build-photo-grade-real-smoke.mjs`，并接入 `image-structured:photo-grade-readiness-building-group`、`:second-sample`、`:real-sample`。三份报告分别输出到 `examples/building-group/proposal-qa-candidates/photo-grade-readiness-report.json`、`examples/building-group-second/photo-grade-readiness-report.json`、`examples/building-real-photo-smoke/photo-grade-readiness-report.json`。当前 building-group candidate 与 second sample 都是 `photo_grade_readiness=technical_baseline`、`photo_grade_candidate=false`；real-photo smoke scaffold 在没有真实建筑照片资产前为 `review_required`。`npm run test:image-structured` 已覆盖六类 gate、parking-only scale、bbox-only parking、top-view overlay 负例、oblique/facade 和 promotion 降级逻辑。
- **2026-06-01**：VisualRelationGraph / Grounding v2 第一刀完成：`projects/image-structured-modeler/scripts/lib/visual-relations.mjs` 会从 ObservationSet 生成 image-space relation candidates，覆盖左右/上下/包含/对齐/接触/同排/镜像对/居中关系，并写入 `visual_relation_graph` 与 `evidence_graph.visual_relations`；PartGraph 生成器会把可判定的方向/包含关系写入 spatial `physical_relations`。新增 `image-structured:visual-relation-switch`、`image-structured:geometry-fit-switch` 与 `examples/switch-controller/visual-relations.fixture.json`，当前 Switch fixture 检查 6 条关系、8 个 contour/keypoint footprint、0 issues，并用 `mirror_x` 负例打回左右手柄、摇杆手性、ABXY vs D-pad 三类关系。建筑群 observations 当前还会从 top view 通过 `pixel_color_segmentation` 提取蓝屋顶与四个白色仓库屋顶 footprint，通过 `pixel_line_segmentation` 提取两条停车线，通过 `pixel_gap_segmentation` 提取负空间 drive aisle，通过 `pixel_vegetation_segmentation` 提取树列，避免继续用 layout prior 的宽仓库框或停车模板作为落位依据；candidate visual fixture 会检查 12 条关系和 9 个 pixel footprint 的模型投影 center/extent。`image-structured:proposal-review-chain-building-group-candidates` 接受 4 条 candidate proposal，把两条 warehouse row 转为 `compile.emit=false` reference container，并新增 8 个 `manual_confirmed` candidate parts；layout/reference/visual relation/GeometryFit/physical QA 均 `pass`，GeometryFit 检查 9 个 footprint、12 条 projected relation、4 个 scale anchor 和 1 个 handedness negative，grounding issues 0。live queue artifact 已刷新为 `output/image-structured-building-group-candidate-warehouses.skp`，226653 bytes，20 groups / 292 faces / 524 edges / 272 vertices。已验证：`npm run image-structured:proposal-review-chain-building-group-candidates`、`npm run image-structured:proposal-review-chain-building-group-candidates:queue`、`npm run image-structured:visual-relation-building-group`、`npm run image-structured:visual-relation-switch`、`npm run image-structured:geometry-fit-switch`、`npm run test:image-structured`、`npm test`、`git diff --check`。
- **2026-05-31**：R7 建筑群 final detail gate 完成：新增 Reference Visual QA `features` 规则族与 schema/test 覆盖，`examples/reference-visual-qa/building-group-r7-final.json` 会检查 accepted roofline/facade/opening feature presence/count 并把 correction target 指向 `parts[...].feature_intents`。新增 `parameter-proposal-review.accepted-all.json`、`correction-patch.detail-proposals-all.json`、`part-graph.r7-final.json`、`output.r7-final.json`、`proposal-review-r7-final/index.html`、`layout-qa-r7-final/`、`reference-visual-qa-r7-final/`、`proposal-qa-r7-final/` 和 `proposal-qa-r7-final-queue/`。`image-structured:proposal-review-chain-building-group-all` mock pass，final DSL 为 55 ops（9 个 `add_raised_rib`、13 个 `cut_recess`、1 个 `cut_slot`），layout / feature Reference Visual QA / physical consistency 均为 `pass`，`review_required=false`。`image-structured:proposal-review-chain-building-group-all:queue` live pass，保存 `output/image-structured-building-group-r7-final.skp`，253,371 bytes；queue snapshot 为 14 groups / 609 faces / 1442 edges / 884 vertices。
- **2026-05-31**：R7 建筑群 live queue QA 完成：SketchUp 2026 Bridge capability handshake 为 `compatibility.ok=true`，插件版本 `queue-plugin-0.1.0-phase7-boolean-manifold.3`。新增 `image-structured:qa-building-group:queue`，当前 queue layout QA 与 Reference Visual QA 均为 `pass` / 0 issues，并生成 `projects/image-structured-modeler/examples/building-group/layout-qa-queue/`、`reference-visual-qa-queue/` 和 `projects/image-structured-modeler/examples/building-group/QUEUE_VERIFICATION.md`。保存的 `.skp` 为 `output/image-structured-building-group-massing.skp`，208,737 bytes；queue snapshot 为 14 groups / 256 faces / 452 edges / 224 vertices / 2 scenes。
- **2026-05-31**：R7 建筑细节 proposal 第一刀完成：`generate-part-graph-from-observations.mjs` 现在会为 `primary_blue_roof_hall`、两组 warehouse、`utility_building` 和 `admin_office` 生成 5 条 review-gated `feature_intents` proposals，覆盖 roofline ribs、facade rhythm、loading/service doors、window bands 和 louver/slot candidates。新增 `image-structured:proposal-review-chain-building-group` 与 `:queue`，当前 fixture 只接受 `primary_blue_roof_hall` 的 detail subset，生成 `correction-patch.detail-proposals.json`、`part-graph.detail-proposal-applied.json`、`output.detail-proposal-applied.json`、`proposal-review/index.html`、`proposal-qa/report.json` 和 `proposal-qa-queue/report.json`；mock 与 queue compile freshness、layout QA、Reference Visual QA 和 physical consistency 均为 `pass` / 0 issues。queue 版保存 `output/image-structured-building-group-detail-proposal-applied.skp`，221,357 bytes；snapshot 为 14 groups / 334 faces / 668 edges / 368 vertices。
- **2026-05-31**：R7 建筑群 mock QA 收口：新增 `examples/model-qa/building-group-massing.json` 与 `examples/reference-visual-qa/building-group-massing.json`，并加入 `image-structured:qa-building-group-layout`、`image-structured:qa-building-group-reference`、`image-structured:qa-building-group` 和 `image-structured:review-building-group`。当前 `projects/image-structured-modeler/examples/building-group/layout-qa/building-group-massing/report.json` 为 `pass` / 0 issues，覆盖 site containment、support、intentional road/scale-anchor overlaps 和 massing separation；`reference-visual-qa/building-group-massing/report.json` 为 `pass` / 0 issues，覆盖 top-view placement/extent/area、scale-anchor ratio、height tier 和 orientation。
- **2026-05-31**：R7.0 建筑群输入基线完成：新增 `building_group` observation profile、campus component candidates、top/oblique view gate、building-group view hints 与样例脚本 `npm run image-structured:analyze-building-group`。用户提供的 GPT Image 合成建筑群三视角图已放入 `test/建筑群/`，脚本生成 `projects/image-structured-modeler/examples/building-group/observations.json` 和 3 张 review overlay。当前 evidence graph 覆盖 `site_boundary`、`primary_blue_roof_hall`、`warehouse_row_west`、`warehouse_row_inner`、`tank_farm`、`utility_building`、`admin_office`、`parking_lot` 和 `internal_roads`，缺失视角为 0；orientation 继续 review-gated，避免把合成图坐标直接当成真实 north/up。
- **2026-05-31**：R7 known-element scale + massing PartGraph 第一刀完成：`building_group` scale calibration 改为 `known_site_element_anchors`，从停车位 2.6m 宽/5.2m 深、13 个停车位宽度、6.5m 停车车道和 3.0m 标记人行横道估算厂区约 `130m x 98m`；所有 scale measurements 仍标记 `review_required: true`。新增 `examples/product-profiles/building_group_industrial_campus.json`、`image-structured:part-graph-building-group`、`image-structured:compile-building-group` 和 `image-structured:build-building-group`，生成 `projects/image-structured-modeler/examples/building-group/part-graph.massing.json`（14 parts，含 site slab、蓝顶主厂房、仓库排、utility/admin、停车/道路、两个 tank cylinder 和 4 个 scale anchor marker）以及 `output.massing.json`（32 ops）。`test:image-structured` 会验证所有 massing shape proposal 均保持 review-gated，并能通过 mock build；这仍是 bbox-derived massing，不是建筑 acceptance。
...
- Queue capability handshake：最新主线目标 manifest 为 `2026-07-official-api-expression-r3` / capability `0.1.0-capabilities.10` / plugin `queue-plugin-0.1.0-official-api-expression-r3`，supported operations `91`，component scope `56`。live queue R3 smoke 已刷新：`npm run qa:official-api-r3:queue` 覆盖 followme、mesh/fill、position_material、runtime selection 和 Page 高级字段，输出 `output/python-sdk-official-api-expression-r3-queue.json`，并保存 `output/python-sdk-official-api-expression-r3.skp`。
...
具体计划已固化到 `docs/product-modeling-architecture-refactor-plan.md`。R1/R2 第一版已经完成：新增 `ProductProfile` / `PartGraph` schema，建立 `vehicle_ambulance` profile，并把救护车验收从直接 DSL 生成迁移为 `profile + part graph -> compiler -> JSON DSL`。R3 Reference Visual QA 已完成引擎第一刀并补上尺寸占比规则：当前可以让 layout 合格但比例/关键点/部件尺寸偏离参考图的模型失败，并给出指向 PartGraph 字段的 correction target。R4 子项目闭环也已完成 evidence/correction 管线。救护车第一轮 visual-quality refit 已完成；R5 已把 Switch 和 Fuji profile 化并接入同一质量报告，三样本 mock + live queue 产品 gate 已通过并保存 artifacts。R6 已把 no-seed 图像证据转成 review-gated PartGraph parameter proposals，打通 accepted proposal 到 correction patch 的回写，补上 proposal review HTML，连接 proposal-applied mock/queue QA 复验链，并把 physical consistency QA 的支撑/贴合/接地关系扩到 ambulance、Switch、Fuji 三样例。R7 已完成当前生成图建筑群 technical baseline；Grounding v2 已把 R8 helper-heavy dense detail 和第二建筑群 mock primary gate 纳入回归；下一步是真实照片/oblique facade 边界。
...
3. **重构图像结构化层**：R4 已完成 contour/keypoint、跨图 part matching、尺度校准、证据置信度和 PartGraph correction loop 的 ambulance 技术预览闭环；R6 已把 no-seed evidence 转成 review-gated parameter proposals，把 accepted proposals 接到 correction patch authoring，并补上 proposal review UI 与 mock/queue QA 复验链；R7 已把建筑群 known-scale/massing/detail proposal final gate 接到 mock/live queue。
4. **补 Reference Visual QA**：救护车第一刀已用正交 preview、轮廓比例、关键点、尺寸占比、面积占比和部件相对位置检查覆盖；当前旧 ambulance seed 模型会在视觉质量 gate 下失败，新 PartGraph 通过。Switch 与 Fuji 也已接入同一类门槛，并覆盖 layout QA 之外的关键点/占比/相对位置回归。R7 final 新增 accepted feature presence/count 门禁；下一刀要用这些门槛扩展样本覆盖并继续压低模板/视觉辅助依赖，而不是继续只调单个样例。
