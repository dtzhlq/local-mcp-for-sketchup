...
| `src/capabilities.mjs` | 单一真源 operation registry，85 个 operation 的支持状态/稳定性/schema/component-scope |
...
- Operation registry / runtime contract 单一注册表化：`src/capabilities.mjs` 作为唯一 operation registry，manifest、runtime `operation_support`、schema、component-scope、docs matrix、contract tests 均从 registry 派生；当前 85 个 operation，53 个 component_definition-scoped operation
...
- Queue capability handshake：最新主线目标 manifest 为 `2026-06-building-geometry-r3-quality` / capability `0.1.0-capabilities.8` / plugin `queue-plugin-0.1.0-building-geometry-r3.1`，supported operations `85`，component scope `53`。最近一次 live queue build/save 证据仍是 R2 aggressive slice：`examples/building-geometry-r2-aggressive.json` 已通过并保存 `output/building-geometry-r2-aggressive.skp`；R3 插件安装/重启后需刷新 live capability/capture/build 抽样。
