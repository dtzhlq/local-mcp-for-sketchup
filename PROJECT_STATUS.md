...
- Queue capability handshake：R2 插件已安装并由 SketchUp 2026 live Bridge 加载，manifest 为 `2026-06-building-geometry-r2-aggressive` / capability `0.1.0-capabilities.7` / plugin `queue-plugin-0.1.0-building-geometry-r2.1`，compatibility `ok`，supported operations `85`，component scope `53`；`examples/building-geometry-r2-aggressive.json` queue build/save 已通过并保存 `output/building-geometry-r2-aggressive.skp`。
