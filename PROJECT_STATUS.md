...
- **2026-07-03**：自然迭代体验 R2 新增 `ModificationIntent v1`：`plan_modification_intent` 会把 target resolution、current selection 和 `analyze_selection_geometry` 几何事实转成可审计 edit intent，记录 evidence refs、confidence、`requires_confirmation`、`safe_to_execute`、limitations 和可选 JSON DSL patch。`iterate_model` 现在可直接消费 `intent` / `intent_file`；只有 `ok=true`、`safe_to_execute=true`、`requires_confirmation=false` 才执行，否则写 preview/blocked manifest 并停止。该节点 MCP tools/list 为 32 个工具。live queue smoke 已通过：Face+Edge selection 只生成 preview/blocked intent；top-level group 安全 `set_attribute` intent 可执行并保存 versioned `.skp` artifact。
