...
分支 `codex/cad-surface-kernel`，基于上一轮 `d895706`。这是本地开发候选，尚非正式发布。2026-09-19 已完成文中支持范围内的本机原生验收；详细状态以 acceptance-report.json 为准。
...
- 初次重开发现法线末位导致指纹误报，已修复。2026-09-19 完整重启 SketchUp 并打开同一 SKP 后，五个上下文均重新定位成功，参数来源、关联状态、顶点及面数量与保存前一致；手工编辑的对照件仍保持参数失效。
...
2026-09-19 安全更新：此前本地服务包仍含旧 fast-uri/sharp，且不含后续返回合同修复。安装前须从当前源码重建服务，不能将历史包哈希当成新包验收；见下方开发候选说明。
