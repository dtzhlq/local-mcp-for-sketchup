...
The runner forcibly disables queue mutation, direct expert queue mutation, S1 auto-approval, legacy trusted-copy approval, and Copy Fast mode. Its create-new report is validated against `schema/release-offline-gate-report-v1.schema.json` before writing. The pre-version rc.2-source checkpoint passed 18 / 18; after the rc.3 v2 live capture is installed as the current validator, the same 18-check gate must be rerun for rc.3. A passing offline report records `offline_acceptance=true`, while `live_queue_acceptance` and `release_acceptance` remain false; it never substitutes for the separately scoped final live gate or a new-version artifact signoff.
