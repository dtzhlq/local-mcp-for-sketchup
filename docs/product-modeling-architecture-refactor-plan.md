...
## RhinoMCP-Inspired Mainline Backlog

These are follow-up TODOs from the 2026-06-04 RhinoMCP review. They should be folded into normal mainline slices when they directly improve SketchUp MCP usability or QA evidence. Do not treat RhinoMCP's open script execution model as a replacement for the current safe JSON DSL, mock runtime, queue parity, and review-gated image grounding discipline.

- [ ] Add a queue-side `get_viewport_image` / `capture_view` tool. It should frame standard views or a supplied bbox, support camera/display-mode options where SketchUp exposes them, and return an image artifact plus structured metadata: camera, scene bbox, visible object count, total object count, and an explicit empty/off-screen diagnostic. This should feed review and Reference Visual QA evidence; it should not replace snapshot/layout/reference gates.
- [ ] Add a lightweight user-facing SketchUp MCP command/agent bundle. Mirror the useful RhinoMCP packaging pattern with an inspector/modeler/QA reviewer workflow: start from `get_capabilities`, list or summarize scene state, capture a viewport, build or save only when requested, and always finish substantial queue edits with QA plus a visible artifact. Keep this as packaging and workflow guidance, not a runtime rewrite.
- [ ] Add named queue session diagnostics before considering multi-process work. The first slice should provide `list_sessions` / stale queue detection / clearer crash-or-timeout errors around the existing single SketchUp queue and lock. Only consider true multi-SketchUp slots after the single-session diagnostics are stable on macOS.
- [ ] Consider a gated `run_ruby_expert` / `run_sketchup_command` tool for local debugging and capability discovery. It must be disabled by default, marked destructive, require an explicit env/config opt-in, return stdout/error, and write an audit artifact. It is not an acceptance path and must not bypass DSL schema, operation registry, or QA gates.
