...
- [x] Add a queue-side `get_viewport_image` / `capture_view` tool. Current implementation frames standard views, writes an image artifact, and returns camera/model metadata. It feeds review and Reference Visual QA evidence; it does not replace snapshot/layout/reference gates.
