...
| 层级 | 当前能力 | 证据入口 |
|---|---|---|
| MCP 工具闭环 | Agent Contract v1 分支当前 `src/mcp-server.mjs` `tools/list` 为 41 个工具：保留 rc.2 的 36 个 expert tools，增加 4 个持久化 Gateway tools 和 1 个显式 queue-only `create_queue_handshake`；工具数不等同于 101 个 safe DSL operation | `src/mcp-server.mjs`、`src/bridge.mjs`、`test/mcp-server.mjs` |
| Agent Gateway contract | 11-state durable task、统一 result envelope、stable errors/next_action、opaque artifact integer pagination、success/error idempotency outcome；preflight handshake/queue/policy 失败保持可恢复，authorized execution 后的不确定错误继续 fail closed | `schema/agent-task-v1.schema.json`、`src/agent-gateway.mjs`、`test/agent-gateway.mjs` |
| Fresh queue Session Contract | `session-contract.v1` 只读绑定 MCP/plugin session、document/model identity、model revision、server/plugin/capability versions、queue idle state 与 expiry；所有 live mutating tools 在同一排他锁内 fail-closed 校验，默认测试不触碰 live SketchUp；用户协调的 41-tool live suite 已另行通过 | `schema/session-contract-v1.schema.json`、`src/session-contract.mjs`、`test/session-contract.mjs`、`docs/evidence/agent-contract-v1-live-evidence-2026-07-15.json` |
...
| Existing Model Editing Engine | `prepare_existing_model_edit` / `apply_reviewed_model_edit` 提供 persistent occurrence path、model revision、S1-S4 risk、budget、一次性可信 approval token 和 stale-model gate；Agent review 字符串不能授权。覆盖深层 Group/Instance/Face/Edge 的受控属性、结构、集合、拓扑、feature、same-scope boolean/manifold | `src/existing-model-editing.mjs`、`scripts/validate-existing-model-editing.mjs`、`test/existing-model-editing.mjs` |
...
| Restricted facade expression R3（仓库内部） | R3 历史能力外，新增官方返回合同的 collection erase/transform、Page named flags/bitwise OR/Boolean update，以及同脚本 affine positioned-material arbitrary-point UVQ；35-case corpus 中 29 个 canonical golden、6 个稳定拒绝 | `examples/python-sdk-source-compat/`、`test/python-sdk-source-compat.mjs`、`test/python-sdk-high-value.mjs` |
...
- 用户协调的 `validate-mcp-capability-suite --runtime queue --queue-required`：41/41 tools accounted、101-op registry 与 34-op live fixture 分开报告，live build/iteration Model QA 均 pass，结束后 queue/processing/responses 为 0、无 lock、无新增 crash report；不等于 101 ops 逐项 live 验证。
...
- `src/mcp-server.mjs` 文档核对：当前架构分支 `tools/list` 为 41 个工具（36 expert + 4 Gateway + 1 个显式 queue-only Session Contract handshake）；其中 `plan_modification_intent` / `iterate_model` 属于可审计门控编排层，不代表自动语义建模智能体；两个图片 adapter 只做受审制品校验和 DSL preview，不自动执行。
...
不考虑图像结构化子项目，主线已经接近官方 v1 的工具闭环和常用建模表达体验：工具形状、snapshot、保存、文件 lifecycle、101-op JSON DSL、受限 Python SDK facade P0-P2 和受审既有模型编辑都已覆盖；`rc.2` 的真实 SketchUp queue 仍以 fresh handshake 为发布硬门禁。
