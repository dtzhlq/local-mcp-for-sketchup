...
- `src/mcp-server.mjs` 当前 Agent Contract v1 分支 `tools/list` 共 40 个工具（保留 36 个 expert tools，增加 4 个 Gateway tools）；其中 `plan_modification_intent` / `iterate_model` 属于可审计门控编排层，不应表述为自动语义建模智能体；两个 image artifact adapter 也不执行图片分析或 queue。`prepare_existing_model_edit` / `apply_reviewed_model_edit` 是 persistent-path + revision + risk + trusted-approval 门控，不是任意脚本入口。
...
- mainline 自然迭代 R2 queue smoke：该历史节点 `tools/list` 为 32 个工具，`plan_modification_intent` 会输出 evidence refs、confidence、`requires_confirmation`、`safe_to_execute`、limitations 和可选 JSON DSL patch，`iterate_model` 可消费 `intent` / `intent_file`。当时 live queue 已验证 Face+Edge selection 停在 preview/blocked，top-level group 安全 `set_attribute` intent 可执行并保存 versioned model artifact；当前 rc.2 为 36 tools，但仍需本轮 fresh queue gate。
