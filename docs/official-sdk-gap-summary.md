...
- Expert Mode v1：`src/expert-compiler.mjs` 使用 AST 白名单解释器把受限脚本编译成标准 JSON DSL，再复用现有 runtime；`node test/expert-compiler.mjs` 已验证 `examples/expert-parametric-fixture.js` 产出 19 个 operations、mock build 12 个 component instances / 2 个 groups / 0 warnings，并覆盖拒绝 `require`、超 loop/operation limit、缺 required field、component 内非法 op 和 `while`。
