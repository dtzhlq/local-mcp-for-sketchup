...
- `src/mcp-server.mjs` 当前 `tools/list` 共 36 个工具；其中 `plan_modification_intent` / `iterate_model` 属于可审计门控编排层，不应表述为自动语义建模智能体；两个 image artifact adapter 也不执行图片分析或 queue。`prepare_existing_model_edit` / `apply_reviewed_model_edit` 是 persistent-path + revision + risk/review 门控，不是任意脚本入口。
