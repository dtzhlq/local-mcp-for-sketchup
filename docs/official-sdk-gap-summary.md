...
- `src/mcp-server.mjs` 当前 Agent Contract v1 分支 `tools/list` 共 41 个工具（保留 36 个 expert tools，增加 4 个 Gateway tools 和 1 个显式 queue-only `create_queue_handshake`）；其中 `plan_modification_intent` / `iterate_model` 属于可审计门控编排层，不应表述为自动语义建模智能体；两个 image artifact adapter 也不执行图片分析或 queue。`prepare_existing_model_edit` / `apply_reviewed_model_edit` 是 persistent-path + revision + risk + trusted-approval 门控，不是任意脚本入口。`get_capabilities` 只证明插件可应答；任何 live mutation 还必须通过签名 Session Contract 的 session/document/model revision/expiry/version/capability/idle 校验。
...
- mainline 自然迭代 R2 queue smoke：该历史节点 `tools/list` 为 32 个工具，`plan_modification_intent` 会输出 evidence refs、confidence、`requires_confirmation`、`safe_to_execute`、limitations 和可选 JSON DSL patch，`iterate_model` 可消费 `intent` / `intent_file`。当时 live queue 已验证 Face+Edge selection 停在 preview/blocked，top-level group 安全 `set_attribute` intent 可执行并保存 versioned model artifact；当前 rc.2 保留 36-tool expert surface，Agent Contract 分支总 `tools/list` 为 41，2026-07-15 fresh 41-tool capability suite 已通过。该 suite 的 live fixture 为 34 ops，不等于 101 ops 逐项语义验证。
...
- 完整官方 Python 类行为；`model.entities/materials/layers/definitions/pages/selection/active_view` 等仍是现有 DSL 的安全对象包装。集合 erase=`None` / transform=`Boolean`、Page named flags/bitwise OR/Boolean update 和同脚本 affine positioned Face 任意点只读 UVQ 已有合同测试；动态 active-model UVHelper、非仿射 UVQ、EntitiesBuilder/intersect、Page flags 完整宿主语义、observer/UI/runtime reflection 仍不支持。
...
本项目已经明确不走“点云/AI mesh 直接终局”。图片理解管线仍保持在独立上游子项目；主线只集成了受审制品 adapter，不复制分析管线也不自动执行。
...
- 主 MCP 已新增 `prepare_image_modeling_brief` 和 `compile_reviewed_part_graph`；只有路径化、schema-valid、promotion-reviewed 且 PartGraph gates 通过的制品才能生成 DSL preview，默认 `queue_called=false`。自动图像校正、轮廓提取、关键点识别、主线 CAD boolean/manifold 的产品样例复用和大样例复验仍属于后续 polish。
