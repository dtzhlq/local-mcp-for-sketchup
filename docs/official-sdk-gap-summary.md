...
- `src/mcp-server.mjs` 当前 `tools/list` 共 32 个工具；其中 `plan_modification_intent` / `iterate_model` 属于可审计门控编排层，不应表述为自动语义建模智能体。
...
- mainline 自然迭代 R2 queue smoke：`src/mcp-server.mjs` 当前 `tools/list` 为 32 个工具，`plan_modification_intent` 会输出 evidence refs、confidence、`requires_confirmation`、`safe_to_execute`、limitations 和可选 JSON DSL patch，`iterate_model` 可消费 `intent` / `intent_file`。live queue 已验证 Face+Edge selection 停在 preview/blocked，top-level group 安全 `set_attribute` intent 可执行并保存 versioned model artifact。
