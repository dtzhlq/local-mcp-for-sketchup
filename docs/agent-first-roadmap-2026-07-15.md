...
> **2026-07-24 rc.3 集成边界：**产品版本、runtime capability 与 manifest 已分别提升为 `0.1.0-rc.3`、`0.1.0-rc.3-capabilities.1`、`2026-07-agent-contract-rc3.1`，提交 `70be58f` 同步了 Node/Ruby/version contract 与 Copy Fast v2 evidence contract。当前 41 tools、101 operations / 57 component scopes 不变，current-source core 为 `39 / 39`。rc.2 的版本对、发布三件套与 v1 live evidence保持不可变历史；rc.3 尚未安装/完整重启、采集 v2 live、复跑最终离线门禁或生成签名制品，`release_acceptance=false`。
...
> **2026-07-24 发布距离盘点：**版本提升与第一层集成边界已经完成，当前源码核心层为 `39 / 39`。版本提升前的不可变 capture 层 `14 / 23` 与正式离线门禁 `18 / 18` 只作为 pre-version checkpoint；rc.3 还剩一条串行闭环：安装已提交插件并完整重启，在固定 disposable copy 上采集 source/version-bound Copy Fast v2 live evidence，接入严格 validator 后复跑 current-source 与 18 项正式离线门禁，再 create-new-only 生成 rc.3 RBZ/checksum/manifest/signoff。更广的 P2 continuous observer、多 live 域，P3 独立 camera/external-reference 跨模型质量和 P5 多厂商/production approval 属于后续产品化，不阻塞一次明确标注边界的技术预览；跨版本继续按用户决定 deferred。
