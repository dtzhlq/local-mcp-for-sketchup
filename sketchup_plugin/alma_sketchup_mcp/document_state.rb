...
module AlmaSketchupMCP
  ...
  DOCUMENT_STATE_DICTIONARY = 'AlmaSketchupMCP'.freeze
  ...
  class DocumentActivationObserver < Sketchup::AppObserver
    ...
    def onActivateModel(model)
      AlmaSketchupMCP.record_activated_model(model)
    end

    def onNewModel(model)
      AlmaSketchupMCP.record_opened_model(model)
    end
  ...
  def persist_document_state(model, state = nil)
    ...
    raise 'Alma document sidecar exceeds the 1 MB safety limit' if encoded.bytesize > DOCUMENT_STATE_MAX_BYTES
  ...
  def load_persisted_document_state(model)
    state = empty_document_state
    raw = model.get_attribute(DOCUMENT_STATE_DICTIONARY, DOCUMENT_STATE_ATTRIBUTE) if model.respond_to?(:get_attribute)
    ...
    state['warnings'] << {
      ...
      'severity' => 'warn',
      'category' => 'state',
      'message' => "Stored Alma document state was ignored: #{error.message}",
