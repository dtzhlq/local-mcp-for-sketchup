...
module LocalMcpForSketchUp
  ...
  def annotate_group(group, operation, fallback_kind)
    ...
    if group.respond_to?(:set_attribute)
      ...
      group.set_attribute('LocalMcpForSketchUp', 'id', id)
      group.set_attribute('LocalMcpForSketchUp', 'kind', kind) if kind
      qa = qa_metadata(operation)
      group.set_attribute('LocalMcpForSketchUp', 'qa', JSON.generate(qa)) if qa
    end
    group
  end
  ...
  def group_kind(group)
    ...
    compatible_attribute(group, ATTRIBUTE_DICTIONARY, 'kind')
  end

  def entity_qa(entity)
    return nil unless entity.respond_to?(:get_attribute)

    raw = compatible_attribute(entity, ATTRIBUTE_DICTIONARY, 'qa')
  ...
  def entity_attributes(entity)
    ...
    entity.attribute_dictionaries.each do |dictionary|
      ...
      dictionary.each_pair do |key, value|
        managed_dictionary = [ATTRIBUTE_DICTIONARY, LEGACY_ATTRIBUTE_DICTIONARIES[ATTRIBUTE_DICTIONARY]].include?(dictionary.name)
        next if managed_dictionary && %w[id kind qa object_transform_json].include?(key.to_s)
  ...
  def entity_object_transform(entity)
    ...
    raw = compatible_attribute(entity, ATTRIBUTE_DICTIONARY, 'object_transform_json')
    return nil if raw.nil? || raw.to_s.empty?
