...
module LocalMcpForSketchUp
  ...
  def feature_base_bounds(entity)
    raw = compatible_attribute(entity, 'LocalMcpFeatures', 'base_bounds_json')
    if raw && !raw.to_s.empty?
      parsed = JSON.parse(raw.to_s)
      return {
        min: feature_bound_tuple(parsed.fetch('min'), 'LocalMcpFeatures.base_bounds_json.min'),
        max: feature_bound_tuple(parsed.fetch('max'), 'LocalMcpFeatures.base_bounds_json.max')
      }
    end
    bounds = feature_bounds(entity)
    entity.set_attribute('LocalMcpFeatures', 'base_bounds_json', JSON.generate('min' => bounds[:min], 'max' => bounds[:max])) if entity.respond_to?(:set_attribute)
    bounds
  ...
  def record_feature_operation(entity, feature)
    ...
    features << feature
    entity.set_attribute('LocalMcpFeatures', 'operations_json', JSON.generate(features))
    entity
  end
  ...
  def entity_features(entity)
    ...
    raw = compatible_attribute(entity, 'LocalMcpFeatures', 'operations_json')
