...
> **2026-07-24 rc.3 发布候选：**当前源码产品版本为 `0.1.0-rc.3`，runtime capability 为 `0.1.0-rc.3-capabilities.1`，manifest 为 `2026-07-agent-contract-rc3.1`；共享 registry 为 **41 MCP tools**（36 expert + 4 Agent Gateway + 1 Session Contract），safe JSON DSL 为 **101 operations / 57 component-definition scope**。22-file 插件已原子安装并在 SketchUp 26.2.242 完整重启；固定 disposable copy 的 `copy-fast-session-live.v2` S4 gate 证明 0 challenge、1 submit、finalized receipt、幂等重放无重复修改、磁盘字节不变、queue 全清，公共 evidence SHA-256 为 `1357a42f…b56e2`。随后 current-source core **39 / 39**、正式离线门禁 **18 / 18** 通过，报告 SHA-256 为 `2f087378…9dba4`。create-new-only 的 rc.3 RBZ/checksum/manifest 已生成并校验，RBZ SHA-256 为 `b5b4dcc4…7b8f2`，manifest 状态为 `rc_candidate_verified` / `rc_signed=true`。这是边界明确的技术预览候选签名，不扩展为完整 SketchUp API、宽泛自动视觉建模、任意 Python、跨版本或一般可用性声明；详见 [rc.3 签署记录](docs/release-candidate-0.1.0-rc.3.md)。
...
- 建筑 helper：`floor_slab`、`footprint_slab`、`wall`、`wall_path`、`curved_wall`、`roof_footprint`、`hip_roof`、`parapet_path`、`curtain_wall`、`column_grid`、`path_surface`、`terrain_mesh`、`parking_stall_array`、`door`、`window`、`stairs`、`railing`、`gable_roof`、`shed_roof`。
...
`validate_reference_model` 是 layout QA 之后的参考视觉 QA：它复用正交 preview renderer，忽略 `reference_image` 图板，检查生成模型自身在参考视图里的 silhouette aspect、keypoint、extent ratio、area ratio、relative placement、orientation/chirality，以及已接受面级 detail 的 `features` presence/count。报告结构与 `validate_model` 保持一致，但 correction suggestions 使用 `action: "update_part_graph"`，目标路径指向 `parts[...].shape.parameters...` 或 `parts[...].feature_intents`。救护车规则在 `examples/reference-visual-qa/ambulance-reference.json`，Switch 规则在 `examples/reference-visual-qa/switch-controller-reference.json`，Fuji 相机规则在 `examples/reference-visual-qa/fuji-camera-reference.json`，建筑群 R7 final detail 规则在 `examples/reference-visual-qa/building-group-r7-final.json`，`npm run qa:reference-visual` 会输出 JSON/Markdown/preview 到 `output/reference-visual-qa/`。当前 ambulance 规则已经从旧 seed geometry 切到参考图锚定门槛；旧快照会失败，新 PartGraph/DSL 通过 mock 和 queue 的 layout/reference QA。Switch 与 Fuji 规则验证了 layout QA 之外的关键点、占比、相对位置和左右手性门槛；新增回归会把 Switch 左右摇杆镜像对调，并要求 `reference.orientation_order` 失败。
...
Image Structured Modeler 的 ambulance no-seed skeleton 现在会在 PartGraph 中输出 `parameter_proposals`：每条提案记录目标 PartGraph path、当前值、建议值、置信度、图像尺度/bbox/keypoint 依据和 `review_required` 状态。当前 no-seed skeleton 覆盖 7 个 profile-required parts、20 条提案，全部保持 review-gated；seed generated PartGraph 记录 45 条提案，质量报告会统计 `parameter_proposals`、`parameter_proposal_parts` 和 `review_required_parameter_proposals`。图像 observation 现在也会记录 `orientation_hints` 和 `visual_relation_graph`：前者包含 `image_x_right_y_down` 坐标约定、mirror risk、semantic anchors 和 `review_required`，后者把 image-space 的左右/上下/包含/对齐/接触/同排/镜像对/居中关系写成带 source image、bbox/keypoint/semantic-anchor basis、confidence 和 `review_required` 的候选证据，避免把 CV 视角分类直接当成模型手性事实。`npm run image-structured:visual-relation-switch` 会用 Switch fixture 对比 observation relation candidates 与 accepted Switch PartGraph 投影，并用 `mirror_x` 负例验证左右镜像会被打回。`npm run image-structured:proposal-patch-ambulance` 会读取 `parameter-proposal-review.accepted.json`，把已接受 proposal 转成 `correction-patch.parameter-proposals.json`，并生成应用后的 `part-graph.proposal-applied.json`。`npm run image-structured:proposal-review-ambulance` 会生成 `projects/image-structured-modeler/examples/ambulance/proposal-review/index.html`，用于勾选 proposal 并导出 accepted proposal JSON。`npm run image-structured:proposal-review-chain-ambulance` 会把 review -> patch -> `output.proposal-applied.json` -> mock QA 串起来；`npm run image-structured:proposal-review-chain-ambulance:queue` 会用 live SketchUp queue 复验并保存 `output/image-structured-ambulance-proposal-applied.skp`。当前只接受 body/cab 两项 proposal 时，该链路应输出 `review_required: true`，layout/reference QA fail，physical consistency pass，防止把部分确认的 no-seed skeleton 误升格为完整产品 acceptance。

R7.0 建筑群输入基线入口是 `npm run image-structured:analyze-building-group`：它读取 `test/建筑群/` 的三张 GPT Image 合成航拍图，使用 `projects/image-structured-modeler/examples/building-group/view-hints.json` 固定 top/oblique 视角，输出 `examples/building-group/observations.json` 和 `review-overlays/*-overlay.png`。当前 evidence graph 确认厂区输入证据：site boundary、蓝顶主厂房、仓库排、罐区、utility building、office、parking lot、internal roads，并额外记录四栋仓库、停车线/车道和树列的候选证据；尺度与 orientation/north-up 仍需要 review。VisualRelationGraph 会记录建筑群 image-space 关系候选，并在 massing PartGraph 中回灌到 `evidence_graph.visual_relations`、part `relationships`、part `physical_relations` 和 review-gated `part_candidates` proposal queue。`npm run image-structured:visual-relation-building-group` 当前检查 12 条关系 / 0 issues，并用 `mirror_x` 负例覆盖蓝顶厂房/仓库/罐区左右手性。`npm run image-structured:proposal-review-chain-building-group-candidates` 会接受四栋仓库、停车线/车道和树列候选，把 `warehouse_row_west` / `warehouse_row_inner` 转为不编译的 reference container，并输出 8 个真实 candidate parts；候选几何使用 top-view `pixel_color_segmentation`、`pixel_line_segmentation`、`pixel_gap_segmentation` 和 `pixel_vegetation_segmentation` footprint，ObservationSet 会为这些候选保存 mask polygon / sampled contour。candidate QA 会检查 12 条视觉关系和 9 个 pixel footprint 与模型投影中心/尺寸匹配，并新增 GeometryFit gate：`proposal-qa-candidates/geometry-fit-report.json` 输出 top-view affine camera/site projection calibration，以及 center/extent/relation/scale/handedness residual。当前 mock/live layout/reference/visual relation/GeometryFit/physical QA 均通过；右下角两条停车线、drive aisle 和 tree row 已从 bbox proxy / mixed grounding 改为 mask/gap/contour-grounded evidence。live queue artifact 为 `output/image-structured-building-group-candidate-warehouses.skp`，226653 bytes，20 groups / 292 faces / 524 edges / 272 vertices。
...
R7 建筑细节 proposal 第一刀仍保留为 partial review 回归：`npm run image-structured:proposal-review-chain-building-group` 只接受 `primary_blue_roof_hall` 的 detail subset，生成 `correction-patch.detail-proposals.json`、`part-graph.detail-proposal-applied.json`、`output.detail-proposal-applied.json`、`proposal-review/index.html` 和 `proposal-qa/report.json`；`:queue` 已保存 `output/image-structured-building-group-detail-proposal-applied.skp`（221357 bytes，14 groups / 334 faces / 668 edges / 368 vertices）。

R7 final detail gate 是 `npm run image-structured:proposal-review-chain-building-group-all`：`parameter-proposal-review.accepted-all.json` 接受 5 个当前 roofline/facade/opening proposal target，生成 `correction-patch.detail-proposals-all.json`、`part-graph.r7-final.json`、`output.r7-final.json`、`proposal-review-r7-final/index.html`、`layout-qa-r7-final/*`、`reference-visual-qa-r7-final/*` 和 `proposal-qa-r7-final/report.json`。最终 DSL 为 55 ops，包含 9 个 `add_raised_rib`、13 个 `cut_recess` 和 1 个 `cut_slot`；mock compile/layout/feature Reference Visual QA/physical consistency 均为 `pass`，`review_required=false`。`npm run image-structured:proposal-review-chain-building-group-all:queue` 已通过 live queue，保存 `output/image-structured-building-group-r7-final.skp`（253371 bytes，14 groups / 609 faces / 1442 edges / 884 vertices）。
...
- 当前只能声明为技术预览：儿童房、救护车、Switch 手柄和 Fuji 相机验收说明系统能表达和保存结构化展示模型；主线已补受控面级 feature operations、CAD boolean/manifold 和 active model 防护，子项目已补 graph-based semantic fusion、corrections workbench 和 R4 evidence/correction loop。救护车已完成第一轮参考图锚定 PartGraph refit，Switch 和 Fuji 已进入 R5 ProductProfile/PartGraph/Reference Visual QA 样本报告，三样本 mock + live queue 批量报告已通过；R6 proposal review 链路已证明部分接受的 no-seed proposal 会被 mock/queue QA 留在 review-gated 状态；R7 已接入建筑群输入 evidence/overlay、known-element scale anchors、review-gated massing PartGraph、mock/live queue layout/reference QA，以及 accepted-all roofline/facade/opening detail proposal -> patch -> feature Reference Visual QA -> mock/live queue QA 链。照片/多图到可编辑模型仍需要继续推进更多产品/建筑边界样例，以及降低模板/视觉辅助比例。
...
- [x] 建筑几何跃迁第一 slice：新增 `footprint_slab` 和 `wall_path`，`wall` 支持非轴向中心线墙；样例为 `examples/building-geometry-slice.json`，mock-first 验收已通过。
- [x] 建筑几何 R2 aggressive slice：`footprint_slab` 支持 holes，非轴向 `wall` / `wall_path` 支持 segment-local openings，并新增 `curved_wall`、`roof_footprint`、`hip_roof`、`parapet_path`、`curtain_wall`、`column_grid`、`path_surface`、`terrain_mesh`、`parking_stall_array`；样例为 `examples/building-geometry-r2-aggressive.json`。
- [x] 建筑几何 R3 quality slice：`curtain_wall` 从最小墙带升级为 module grid，支持 `row_count`、`panel_thickness`、vertical mullions、horizontal rails 和 panel cells；operation 数保持 85。
...
- [x] Image Structured VisualRelationGraph 第一刀：ObservationSet 会输出 image-space relation candidates，Switch 人工视觉检查已内化为 `visual-relations.fixture.json` + `image-structured:visual-relation-switch`，建筑群 fixture 已覆盖四栋仓库/停车/树列关系，并把细粒度候选写入 PartGraph review-gated candidate proposal queue；四栋仓库、停车线/车道和树列 candidate promotion 已接入 pixel-grounded footprint QA，两条 fixture 都有 `mirror_x` 负例。
- [x] Perception-to-Geometry Grounding v2：ObservationSet/EvidenceGraph/PartGraph 记录 grounding provenance，pixel-grounded building-group candidates 记录 mask polygon / sampled contour，Switch front controls 记录 edge contour/keypoint grounding，`validate-geometry-fit.mjs` 输出 camera/site projection calibration、footprint/relation/scale/handedness residual 和 dense-detail helper/review report；当前 mock/live queue 已通过 building-group candidate GeometryFit，R8 no-texture 模型会被标为 helper-heavy review，第二个生成建筑群样本已通过 mock primary gate。
...
- [x] R7 building-group final detail gate：`image-structured:proposal-review-chain-building-group-all` / `:queue` 已完成 accepted-all roofline/facade/opening proposals -> patch -> feature Reference Visual QA -> mock/live queue QA，并保存 `output/image-structured-building-group-r7-final.skp`。
