import {legacyToolOutputSchema} from './tool-output-contracts.mjs';
...
export const MODELING_TOOL_DEFINITIONS=[
  {name:'query_model_geometry',description:'Read revision-bound vertices, edges, faces, loops and occurrence transforms for specified model contexts. Returns a compact summary and immutable resource handle; detail=full is explicit. Use page_size then cursor for frozen topology pages. Read-only.',inputSchema:{type:'object',additionalProperties:false,properties:{...shared,targets,snapshot_handle:handle,page_size:{type:'integer',minimum:1,maximum:10000,description:'Page size in topology records; captures a frozen snapshot up to two million vertices.'},cursor:{type:'string',maxLength:300,description:'Opaque continuation cursor; bound to the immutable snapshot, never the current model.'},recursive:{type:'boolean',default:true},detail:{enum:['summary','full'],default:'summary'}}}},
  ...
  {name:'edit_model_geometry',description:'Submit one revision-bound topology batch in model (loose root geometry) or a group/component context: add edges/faces, move vertices, transform, pushpull, reverse, erase or set properties. Also supports sole replace_cad edits for current parametric CAD sources, including NURBS control points and curved-edge fillet recipes. Preserves sibling instances through isolation. Uses existing reviewed tasks and receipts; returns approval steps when required.',inputSchema:{type:'object',additionalProperties:false,required:['snapshot_handle','entity_path','edits','idempotency_key'],properties:{runtime,snapshot_handle:handle,entity_path:{type:'string',pattern:'^(model|pid:[1-9][0-9]*(\\.[1-9][0-9]*)*)$'},edits:{type:'array',minItems:1,maxItems:100,items:GEOMETRY_EDIT_SCHEMA},idempotency_key:{type:'string',minLength:1,maxLength:200},instruction:{type:'string'}}}},
...
export function toolOutputSchema(name) {
  ...
  return legacyToolOutputSchema(name);
}
