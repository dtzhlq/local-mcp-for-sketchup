import { MODELING_TOOL_DEFINITIONS, toolOutputSchema } from './modeling-tools.mjs';
...
export const TOOL_REGISTRY = Object.freeze(BASE_TOOL_REGISTRY.map((tool) => {
  ...
  const readOnly = new Set(['query_model_geometry','measure_model_geometry','query_assets','get_docs','get_workflow_bundle','get_capabilities','queue_diagnostics','compile_expert','compile_python_sdk','get_model_info','list_entities','inspect_model','resolve_model_targets','get_selection','analyze_selection_geometry','compare_snapshots','inspect_detail_regions']).has(tool.name);
  return {
    ...tool,
    outputSchema: toolOutputSchema(tool.name),
    annotations: {readOnlyHint:readOnly,destructiveHint:!readOnly&&!new Set(['create_queue_handshake','capture_view','capture_detail_views','prepare_existing_model_edit','prepare_image_modeling_brief','compile_reviewed_part_graph']).has(tool.name),idempotentHint:readOnly,openWorldHint:tool.name==='run_ruby_expert'},
    inputSchema: {
