...
\`adopt_open_model\` is the bridge from arbitrary local SketchUp models into the safe editing loop. It writes Alma adoption attributes only on top-level groups and component instances, preserving existing MCP ids unless \`force: true\` is supplied, and returns an entity list with stable references. With \`recursive: true\`, it also returns a read-only nested index so component internals can be inspected before future definition-scoped editing; this slice edits top-level groups/component instances only.
