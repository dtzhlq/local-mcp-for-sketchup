...
CAD surface candidate (local OpenCascade WebAssembly):
- cad_shape is an additive DSL operation: {op:"cad_shape",name,recipe:{version:1,nodes:[...],output:"nodeId"},tolerance:0.2,angular_tolerance:0.35}. Units mm; no arbitrary kernel code. Also available as Python model.cad_shape(options).
- Each node has unique id and kind. box: min/max; cylinder: radius/height, optional origin/axis; nurbs_surface: poles[u][v][xyz], optional positive weights[u][v], u_degree/v_degree, unique u_knots/v_knots and matching u_multiplicities/v_multiplicities. Supports clamped nonperiodic rectangular patches, degrees 1..8, grid 2..32 by 2..32.
- sew: inputs (prior face/shell node ids), solid boolean; an open shell cannot become a solid. fuse/cut/intersect: input and other prior node ids. fillet: input, radius and optional edges (indices from the exact input recipe's CAD edge inventory). Multi-edge corner blends may fail validity checks; do not reuse indices after changing upstream topology without recomputing the input inventory.
- query_model_geometry(detail=full) returns context.cad with recipe, source_hash, current flag, and exact kernel evidence. Native faces expose cad_face patch correspondence. Default query only advertises compact CAD availability; use its frozen geometry resource for details.
- To update one parametric object, use edit_model_geometry with sole edit {op:"replace_cad",recipe,...meshOptions}, current snapshot_handle, entity_path and stable idempotency_key. Keep coordinates local. Updates use existing approval, instance isolation and atomic transactions. Modified native geometry makes cad.current false and regeneration is rejected; do not overwrite those edits.
- A recipe has at most 24 nodes; compute worker has a 30 s deadline; mesh budgets max 50,000 vertices/faces. Invalid geometry is rejected before native edits. Meshing tolerance is a kernel tessellation setting, not an independently certified Hausdorff error bound. Native measurements refer to the displayed mesh; cad.evidence.volume_mm3 refers to the exact CAD solid.
- This does not fit arbitrary old meshes to NURBS, support trimmed NURBS domains/periodic control grids, or guarantee every fillet radius/junction.
