...
export const MODEL_ACCESSIBILITY_TASK_KINDS = Object.freeze(['window', 'door', 'cabinet', 'sink_counter', 'asset_placement']);
...
const TASK_TITLES = { window: 'Detailed double sash window', door: 'Detailed left hinged door', cabinet: 'Three drawer cabinet', sink_counter: 'Countertop with real sink opening', asset_placement: 'Place complete built-in asset roots' };
...
export function preflightModelAccessibilityTask(task, context = {}) {
  ...
  if (task.kind === 'asset_placement') {
    ...
    if (!asset || !Object.hasOwn(COMMON_DIMENSIONS, asset) || asset === 'sink_counter') issue(issues, 'task.asset_id', task.asset_id === undefined ? 'MISSING_INPUT' : 'UNSUPPORTED_ASSET', 'Select an asset_id whose catalog entry has placement_supported=true.', 'Read the built-in asset catalog and use its exact detail-<name> id.');
    recipeKind = asset;
  ...
  if (requireObject(task.parameters, 'task.parameters', issues) && contract) {
    ...
    for (const [key, rule] of Object.entries(contract)) {
      ...
      else if (rule.type === 'number' && (!Number.isFinite(value) || value < rule.minimum || value > rule.maximum)) issue(issues, `task.parameters.${key}`, 'OUT_OF_RANGE', `${key} must be a finite number in [${rule.minimum}, ${rule.maximum}] ${rule.units || ''}.`, `Supply ${rule.description}`);
  ...
  if (!issues.length) {
    try {
      ...
      const budget = Object.fromEntries(Object.entries(DETAIL_SAMPLE_RESOURCE_BUDGET).map(([key, value]) => [key, Math.min(value, context.resourceBudget?.[key] ?? value)]));
      ...
      if (context.limits?.max_operations !== undefined && operationCount > context.limits.max_operations) issue(issues, 'task.kind', 'RESOURCE_BUDGET', `Generated operation count ${operationCount} exceeds server limit ${context.limits.max_operations}.`, 'Use a smaller explicitly scoped task or request a reviewed server policy change.', 'runtime_blocked');
      if (context.limits?.max_affected_instances !== undefined && bundle.parts_mapping.length > context.limits.max_affected_instances) issue(issues, 'task.instances', 'RESOURCE_BUDGET', `Expanded leaf occurrence count ${bundle.parts_mapping.length} exceeds server limit ${context.limits.max_affected_instances}.`, 'Reduce the number of requested instances.', 'runtime_blocked');
      normalized.resource_estimate = estimates;
      normalized.operation_count = operationCount;
      normalized.leaf_occurrence_count = bundle.parts_mapping.length;
  ...
  return result();
  function result() { return { kind: 'model_accessibility_preflight', version: MODEL_ACCESSIBILITY_TASK_VERSION, ok: issues.length === 0, issues, normalized: issues.length ? null : normalized, evidence_level: 'preflight_only', execution_started: false, quality_accepted: false, boundaries: clone(BOUNDARIES), remaining_runtime_checks: ['fresh runtime capability and session binding', 'atomic target/name absence and execution policy', 'actual geometry, collision and required void measurements', 'native resource totals', 'server close views and file save/reopen'], checked: ['declared dimensions and dependencies', 'units, axes and placement', 'references and names supplied in context', 'all generated operation dependencies when supplied', 'conservative expanded geometry estimate'] }; }
}
...
export function compileModelAccessibilityTask(task, context = {}) {
  ...
  const bundle = buildBundle(preflight.normalized);
...
function buildBundle(task) {
  const parameters = Object.fromEntries(Object.entries(task.parameters).map(([key, value]) => [PARAMETER_NAMES[key] || key, value]));
  const recipe = task.kind === 'sink_counter' ? sinkCounterRecipe(task.id, task.parameters) : FURNISHING_ASSET_KINDS.includes(task.recipe_kind) ? buildFurnishingAsset(task.recipe_kind, { id: task.id, parameters }) : buildDetailedRecipe(task.recipe_kind, { id: task.id, parameters });
  ...
  const occurrences = describeAssemblyOccurrences(graph);
  const required_parts = requirementsForOccurrences(graph.parts, occurrences, recipe.requirements);
  const required_voids = bindRecipeVoidsToInstances(recipe.required_voids || [], recipe.root_id, task.instances);
  const views = task.instances.flatMap(instance => camerasFor(task, instance));
  ...
  return { dsl, part_graph: graph, profile, parts_mapping: occurrences, detail_spec, views, recipe_signature: recipe.recipe_signature, parameters: clone(task.parameters), source: 'explicit_task_compilation', evidence_level: 'constructive_dsl_only', live_geometry_verified: false };
}
...
function estimateGeometry(graph) {
  ...
  const costs = id => {
    ...
    if (part.assembly) return part.assembly.children.reduce((sum, child) => add(sum, costs(child.part_id)), { faces: 0, edges: 0, vertices: 0 });
    ...
    return { faces, edges: faces * 3, vertices };
  };
