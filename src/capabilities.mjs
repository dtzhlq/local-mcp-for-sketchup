...
const OPERATION_REGISTRY_ENTRIES = [
  {op:'cad_shape',description:'Exact CAD recipe with rational NURBS surfaces, solids, Boolean operations and curved-edge fillets; bounded local kernel emits editable SketchUp mesh.',schema:{required:['op','name','recipe'],optional:['id','material','transform','tolerance','angular_tolerance','max_vertices','max_faces']},runtime_support:{mock:SUPPORT_STATUS.supported,queue:SUPPORT_STATUS.supported},stability:STABILITY.experimental,component_definition:true,notes:'Recipe retained; not arbitrary mesh-to-NURBS recovery. Kernel failures do not mutate the model.'},
