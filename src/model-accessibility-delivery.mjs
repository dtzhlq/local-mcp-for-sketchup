...
async function sourceBinding(source, taskStore, bridge) {
  ...
  let frozen = creation.frozen_spec;
  if (!frozen && source.inputs?.source_image_model) {
    const { assertFrozenImageCreation, imageModelDetailSpecification } = await import('./image-structure.mjs');
    await assertFrozenImageCreation(bridge.agentGateway, source);
    frozen = freezeDetailSpecification(imageModelDetailSpecification(source.private.image_model_creation.plan));
    quality = evaluateDetailQuality({ snapshot: creation.round.snapshot, layoutQa: result.qa,
      frozenSpecification: frozen, runtime: 'queue', trustedSnapshot: true,
      identityMap: creation.identity_map, materialMap: creation.material_map });
    if (quality.quality_status !== 'pass' || !quality.quality_accepted || quality.remaining.length) {
      fail('OPERATION_NOT_ALLOWED', 'Image delivery requires each frozen emitted part to have native geometry evidence.');
    }
  }
