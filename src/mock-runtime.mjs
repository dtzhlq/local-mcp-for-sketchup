...
function operationLimit() {
  const raw = process.env.LOCAL_MCP_FOR_SKETCHUP_MAX_OPERATIONS;
  if (!raw) return DEFAULT_OPERATION_LIMIT;
  const parsed = Number(raw);
  if (!Number.isInteger(parsed) || parsed < 1) {
    throw new Error('LOCAL_MCP_FOR_SKETCHUP_MAX_OPERATIONS must be a positive integer');
  }
  return parsed;
}

export class MockRuntime {
  constructor({ sessionPath = process.env.LOCAL_MCP_FOR_SKETCHUP_MOCK_SESSION_PATH || mockSessionPath, sourcePath = null, lockTimeoutMs = DEFAULT_LOCK_TIMEOUT_MS } = {}) {
...
export function parseDsl(code) {
  ...
  if (document.operations.length > maxOperations) {
    throw new Error(`DSL operation limit exceeded: max ${maxOperations} operations. Set LOCAL_MCP_FOR_SKETCHUP_MAX_OPERATIONS to raise this for trusted large models.`);
  }
